...
\texttt{etl/raw\_loader.py} & Pré-validação, COPY, recarga substitutiva e reconciliação RAW \\\hline
\rowcolor{CinzaClaro}
\texttt{etl/sql/load\_core.sql} & Transformações SQL na ordem de dependências \\\hline
\texttt{etl/core\_loader.py} & Execução e reconciliação do CORE \\\hline
\rowcolor{CinzaClaro}
\texttt{etl/pipeline.py} & Orquestração atômica CSV $\rightarrow$ RAW $\rightarrow$ CORE \\\hline
