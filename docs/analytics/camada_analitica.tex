...
\newcommand{\CabecalhoTabela}[1]{\textcolor{white}{\textbf{#1}}}
...
\hfill
\textbf{\textcolor{AzulCorporativo}{Data:}} 6 de setembro de 2026
...
analítica}\label{documentacao-analitica}

Documentação consolidada da camada analítica, destinada à leitura humana
e ao consumo por SQL e ferramentas de Business Intelligence.

\subsection{Artefatos}\label{docs__analytics__readme.md__artefatos}

\begin{itemize}
\item
  \href{camada_analitica.tex}{\nolinkurl{camada_analitica.tex}}: fonte
  canônica e editável da documentação consolidada;
\item
  \href{camada_analitica.pdf}{\nolinkurl{camada_analitica.pdf}}: versão
  final para leitura e distribuição.
\end{itemize}

O documento reúne contrato analítico, evidências das consultas,
validação dos requisitos funcionais, catálogo de métricas, estruturas do
schema \nolinkurl{analytics} e contrato de consumo no Power BI e SQL.
...
Este documento consolida o problema de negócio, os stakeholders, as
decisões, as demandas, os casos de uso e o catálogo de indicadores que
orientaram o desenvolvimento da Camada Analítica e Extração de Inteligência.

O contrato antecedeu e orientou a implementação das consultas e
estruturas analíticas, sem transformar as atividades em uma sequência
mecânica. Exploração, validação de requisitos e definição de métricas se
retroalimentaram, com alterações sustentadas por evidência e registradas
de forma rastreável.
...
O cenário analítico partiu de uma empresa operadora de marketplace que
precisava compreender seu desempenho histórico para orientar decisões
financeiras, comerciais e operacionais. Embora possuísse dados integrados
sobre pedidos, clientes, itens, produtos, vendedores, pagamentos,
avaliações e entregas, ainda não dispunha de uma visão analítica
...
\begin{minipage}[b]{\linewidth}\raggedleft\color{white}\bfseries
Prioridade
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright\color{white}\bfseries
...
\bottomrule\noalign{}
\endlastfoot
P0 & Financeiro/\allowbreak executivo & Acompanhar desempenho & Quanto foi
movimentado e como evoluiu? & Valores dos itens, frete, valor bruto,
valor pago e crescimento \\
P0 & Financeiro/\allowbreak executivo & Validar consistência & O valor pago é
...
\toprule\noalign{}
\rowcolor{AzulCorporativo}
\begin{minipage}[b]{\linewidth}\raggedright\color{white}\bfseries
Métrica
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright\color{white}\bfseries
Fórmula
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright\color{white}\bfseries
...
Esta lista registra a formulação preliminar que orientou a exploração. A
disponibilidade, a qualidade, a distribuição e a utilidade foram
validadas antes da consolidação das métricas e estruturas de consumo nas
seções seguintes.
...
Consumo por BI & RF12 & Views e consultas de exportação versionadas \\
Preservação de granularidade & Regra aprovada de joins analíticos &
Consultas de controle e validação \\
\end{longtable}
}

\subsection{Impacto no desenvolvimento
analítico}\label{docs__analytics__contrato_analitico.md__impacto-no-fluxo-m06}

\begin{itemize}
\tightlist
\item
  A elaboração das consultas básicas explorou a CORE a partir das perguntas priorizadas neste
  contrato;
\item
  A elaboração das consultas analíticas combinou entidades para responder aos casos de uso aprovados;
\item
  A validação dos requisitos funcionais verificou a cobertura dos requisitos e registrou evidências;
\item
  A definição das métricas validou e consolidou o catálogo preliminar de métricas;
\item
  A criação das views persistiu somente regras reutilizáveis e semanticamente
  estáveis;
\item
  A preparação do consumo em SQL e Power BI organizou datasets com nomes, granularidade e métricas
  explícitos.
\end{itemize}

O contrato foi refinado durante essas entregas. As mudanças registram a
evidência, a decisão e o impacto sobre consultas, métricas e estruturas
produzidas.
...
durante a implementação das consultas básicas. Os
...
\toprule\noalign{}
\rowcolor{AzulCorporativo}
\CabecalhoTabela{Relação CORE} & \CabecalhoTabela{Registros} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\toprule\noalign{}
\rowcolor{AzulCorporativo}
\CabecalhoTabela{Status} & \CabecalhoTabela{Pedidos} & \CabecalhoTabela{Participação} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\toprule\noalign{}
\rowcolor{AzulCorporativo}
\CabecalhoTabela{Classificação} & \CabecalhoTabela{Pedidos} & \CabecalhoTabela{Participação} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
Este documento registra evidências da elaboração das consultas analíticas, orientada
...
\toprule\noalign{}
\rowcolor{AzulCorporativo}
\CabecalhoTabela{Indicador} & \CabecalhoTabela{Resultado} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
já identificados na exploração das consultas básicas.
...
\begin{minipage}[b]{\linewidth}\raggedright\color{white}\bfseries
Tipo
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft\color{white}\bfseries
Registros
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft\color{white}\bfseries
Pedidos distintos
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft\color{white}\bfseries
...
na granularidade de item é 9,32\%, conforme estabilizado nas views do schema analytics.
...
\subsection{Decisões incorporadas às etapas
seguintes}\label{docs__analytics__evidencias_consultas_analiticas_core.md__decisuxf5es-para-as-pruxf3ximas-etapas}
...
\item
  ...
  associados;
\item
  validar fórmulas e cobertura na validação dos requisitos funcionais;
\item
  consolidar como métricas somente os indicadores aprovados no catálogo analítico.
\end{itemize}
...
sustentados pelo modelo CORE e pelas consultas básicas e analíticas versionadas. A validação comprova capacidade analítica e cobertura
observada; não declara que toda análise possível foi implementada nem que
um arquivo Power BI ou seu projeto visual integrem o escopo entregue.
...
\scriptsize
\rowcolors{2}{CinzaClaro}{white}
\begin{longtable}{|>{\RaggedRight\arraybackslash}p{1.7cm}|>{\RaggedRight\arraybackslash}p{4.1cm}|>{\RaggedRight\arraybackslash}p{5.3cm}|>{\RaggedRight\arraybackslash}p{9.7cm}|>{\RaggedRight\arraybackslash}p{2.6cm}|}
\hline
\rowcolor{AzulCorporativo}
\CabecalhoTabela{Requisito} &
\CabecalhoTabela{Capacidade validada} &
\CabecalhoTabela{Consulta ou artefato} &
\CabecalhoTabela{Evidência observada} &
\CabecalhoTabela{Resultado} \\ \hline
\endfirsthead
\hline
\rowcolor{AzulCorporativo}
\CabecalhoTabela{Requisito} &
...
\nolinkurl{core} e \nolinkurl{analytics}, fontes em
\nolinkurl{models/analytics/views/}, consultas em
\nolinkurl{queries/consumption/} e contrato de consumo & A CORE validada,
quatro views reutilizáveis e quatro consultas de exportação formam a base
documentada para consumo por SQL e Power BI. & Atendido \\
\end{longtable}
}
\end{landscape}
...
\toprule\noalign{}
\rowcolor{AzulCorporativo}
\CabecalhoTabela{Relação validada} & \CabecalhoTabela{Origem} & \CabecalhoTabela{Relacionados} & \CabecalhoTabela{Sem relação} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\subsection{Decisões consolidadas após a
validação}\label{docs__analytics__matriz_validacao_requisitos_funcionais.md__decisuxf5es-e-impacto-nas-pruxf3ximas-etapas}

\begin{itemize}
\tightlist
\item
  RF01 a RF12 possuem cobertura suficiente, respeitadas as limitações
  registradas.
\item
...
\item
  RF12 é classificado como atendido após a criação das estruturas
  reutilizáveis e dos datasets de consumo.
\item
...
\scriptsize
\rowcolors{2}{CinzaClaro}{white}
\begin{longtable}{|>{\RaggedRight\arraybackslash}p{1.1cm}|>{\RaggedRight\arraybackslash}p{4.0cm}|>{\RaggedRight\arraybackslash}p{3.7cm}|>{\RaggedRight\arraybackslash}p{3.5cm}|>{\RaggedRight\arraybackslash}p{3.4cm}|>{\RaggedRight\arraybackslash}p{4.0cm}|>{\RaggedRight\arraybackslash}p{3.8cm}|}
\hline
\rowcolor{AzulCorporativo}
\CabecalhoTabela{ID} & \CabecalhoTabela{Nome e finalidade} &
\CabecalhoTabela{Fórmula e unidade} &
\CabecalhoTabela{Granularidade e tempo} &
\CabecalhoTabela{Filtros e exclusões} &
\CabecalhoTabela{Fontes e SQL} &
\CabecalhoTabela{Interpretação e limitações} \\ \hline
\endfirsthead
\hline
\rowcolor{AzulCorporativo}
\CabecalhoTabela{ID} & \CabecalhoTabela{Nome e finalidade} &
...
\toprule\noalign{}
\rowcolor{AzulCorporativo}
\CabecalhoTabela{Métrica} & \CabecalhoTabela{Resultado} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
regras recorrentes validadas pelas consultas básicas e analíticas, pela
validação dos requisitos funcionais e pelo catálogo de métricas. Elas reduzem
...
materializam dados. O contrato de consumo definiu os campos e relacionamentos de
consumo; a criação de um arquivo Power BI e de seu projeto visual
...
Este documento consolida o contrato de consumo em Power BI e SQL. Ele organiza
...
As quatro views do schema analytics já estabilizam as regras recorrentes e
são as fontes canônicas de consumo. O contrato de consumo não exige marts adicionais
porque:
...
\toprule\noalign{}
\rowcolor{AzulCorporativo}
\CabecalhoTabela{Grupo} & \CabecalhoTabela{Tratamento recomendado} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
