...
def test_pbip_and_bindings_exist():
    ...
    assert binding["datasetReference"]["byPath"]["path"] == "../EcommerceAnalytics.SemanticModel"
    assert json.loads((MODEL / "definition.pbism").read_text(encoding="utf-8"))["version"] == "4.2"


def test_four_canonical_analytics_views_are_sources():
    texts = "\n".join(p.read_text(encoding="utf-8") for p in (MODEL / "definition" / "tables").glob("*.tmdl"))
    for view in ["vw_pedido_financeiro", "vw_financeiro_mensal", "vw_vendedor_pedido", "vw_desempenho_vendedor"]:
...
def test_report_has_five_pages_and_42_visuals():
    ...
    visual_files = list((REPORT / "definition" / "pages").glob("*/visuals/*/visual.json"))
    assert len(visual_files) == 42


def test_drillthrough_and_topn_exist():
    page_texts = [json.loads(p.read_text(encoding="utf-8")) for p in (REPORT / "definition" / "pages").glob("*/page.json")]
    assert any(p.get("pageBinding", {}).get("type") == "Drillthrough" for p in page_texts)
    visuals = [json.loads(p.read_text(encoding="utf-8")) for p in (REPORT / "definition" / "pages").glob("*/visuals/*/visual.json")]
    assert any(any(f.get("type") == "TopN" for f in v.get("filterConfig", {}).get("filters", [])) for v in visuals)
...
def test_no_binary_or_local_powerbi_artifacts_committed():
    ...
    assert not [p for p in BI.rglob("*") if p.is_file() and p.suffix.lower() in forbidden]
